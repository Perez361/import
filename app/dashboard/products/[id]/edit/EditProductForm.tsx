...
export default function EditProductForm({ product, userId }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fd.append('existing_image_url', imageCleared ? '' : (product.image_url || ''))
      ...
      <div className="bg-[var(--color-card)] rounded-2xl border border-[var(--color-border)] p-6">
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            >
              {imagePreview ? (
                ...
                <>
                  ...
                  <p className="text-xs text-[var(--color-text-muted)] mt-1">PNG, JPG, WebP — max {MAX_INPUT_SIZE_MB} MB · auto-optimized</p>
                </>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold mb-1.5 text-[var(--color-text-primary)]">Product Name *</label>
            ...
            <div>
              <label className="block text-sm font-semibold mb-1.5 text-[var(--color-text-primary)]">Price (GH₵) *</label>
          ...
          {/* Description */}
          <div>
            <label className="block text-sm font-semibold mb-1.5 text-[var(--color-text-primary)]">Description</label>
          ...
          <div className="border-t border-[var(--color-border)] pt-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={formData.tracking_number}
                ...
                <p className="text-xs text-[var(--color-text-muted)] mt-1">Latest tracking number from the overseas store</p>
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1.5 text-[var(--color-text-primary)]">Supplier / Store</label>
            ...
            <div className="mt-4">
              <label className="block text-sm font-semibold mb-1.5 text-[var(--color-text-primary)] flex items-center gap-1.5">
                <ExternalLink className="h-3.5 w-3.5 text-[var(--color-brand)]" /> Supplier Product URL
              </label>
              <input type="url" value={formData.supplier_url}
                onChange={(e) => setFormData({ ...formData, supplier_url: e.target.value })}
                className="w-full px-3 py-2.5 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] focus:ring-2 focus:ring-[var(--color-brand)] focus:border-transparent text-sm"
                placeholder="https://amazon.com/dp/..." />
              <p className="text-xs text-[var(--color-text-muted)] mt-1">Link to the product page on the overseas store</p>
            </div>
          </div>
          ...
          <button type="submit" disabled={saving || optimizing}
            ...
            {saving ? <><Loader2 className="h-4 w-4 animate-spin" /> Saving…</> : <><Package className="h-4 w-4" /> Save Changes</>}
          </button>
        </form>
      </div>
    </div>
