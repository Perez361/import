...
const STATUS_CONFIG: Record<string, { label: string; classes: string; icon: React.ReactNode }> = {
  pending:          { label: 'Pending',         classes: 'bg-[var(--color-warning-light)] text-[var(--color-warning)]',   icon: <Clock className="h-3 w-3" /> },
  processing:       { label: 'Processing',      classes: 'bg-[var(--color-brand-light)] text-[var(--color-brand)]',       icon: <Package className="h-3 w-3" /> },
  arrived:          { label: 'Arrived',          classes: 'bg-purple-100 text-purple-600',                                 icon: <Truck className="h-3 w-3" /> },
  shipping_billed:  { label: 'Shipping Billed', classes: 'bg-orange-100 text-orange-600',                                 icon: <DollarSign className="h-3 w-3" /> },
  shipping_paid:    { label: 'Shipping Paid',   classes: 'bg-[var(--color-success-light)] text-[var(--color-success)]',   icon: <CheckCircle2 className="h-3 w-3" /> },
  delivered:        { label: 'Delivered',        classes: 'bg-[var(--color-success-light)] text-[var(--color-success)]',   icon: <PackageCheck className="h-3 w-3" /> },
  cancelled:        { label: 'Cancelled',        classes: 'bg-[var(--color-danger-light)] text-[var(--color-danger)]',    icon: <XCircle className="h-3 w-3" /> },
}
...
export default function OrdersTable({ orders, importerPhone, storeSlug }: Props) {
  ...
  const handleBillShipping = async (order: Order) => {
    ...
    updateLocal(order.id, { shipping_fee: fee, shipping_note: shippingNotes[order.id] || null, status: 'shipping_billed', shipping_billed_at: new Date().toISOString() })
    toast.success('Shipping fee sent!')
  }
  ...
  const handleMarkPaid = async (order: Order) => {
    ...
    if (result?.error) { toast.error(result.error); return }
    updateLocal(order.id, { shipping_paid: true, status: 'delivered', shipping_paid_at: new Date().toISOString() })
  ...
  const whatsappLink = (order: Order) => {
    ...
    const message = encodeURIComponent(
      `Hello ${name}! 👋\n\nYour pre-order has arrived!\n\n📦 *Order #${order.id.slice(-8).toUpperCase()}*\n${items}\n\n` +
      `💰 Product: GH₵${fmt(productTotal)}\n🚚 Shipping: GH₵${fmt(shippingFee)}\n*Total: GH₵${fmt(grandTotal)}*\n\n` +
          ...
          <div key={order.id}>
            ...
            {isExpanded && (
              <div className="px-4 pb-5 sm:px-6 space-y-4 border-t border-[var(--color-border)] bg-[var(--color-surface)]">
                ...
                {/* Status change */}
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)] mb-2">Update Status</p>
                  <div className="flex flex-wrap gap-1.5">
                    {STATUS_FLOW.filter((s) => s !== status).map((s) => (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg text-xs font-medium border border-[var(--color-border)] text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-card)] transition-all disabled:opacity-50"
                      >
                        → {s.replace('_', ' ')}
                      </button>
                    ))}
                  ...
                  <div className="rounded-xl border-2 border-dashed border-orange-200 bg-orange-50 p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          type="text" placeholder="e.g. Pay to 0551234567"
                          value={shippingNotes[order.id] || ''}
                          onChange={(e) => setShippingNotes((prev) => ({ ...prev, [order.id]: e.target.value }))}
                          onClick={(e) => e.stopPropagation()}
                          className="w-full px-3 py-2 rounded-lg border border-[var(--color-border)] bg-white text-sm focus:ring-2 focus:ring-orange-400 focus:border-transparent"
                        />
                      </div>
                  ...
                  <div className="rounded-xl border border-orange-200 bg-orange-50 p-4 space-y-2">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm font-semibold text-orange-700">Awaiting payment</p>
                      </div>
                      {customer?.contact && (
                        ...
                        >
                          <MessageCircle className="h-3 w-3" /> Resend WhatsApp
                        </a>
                      )}
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div><p className="text-xs text-[var(--color-text-muted)]">Shipping</p><p className="font-bold text-orange-600">GH₵{fmt(shippingFee)}</p></div>
                      <div><p className="text-xs text-[var(--color-text-muted)]">Grand Total</p><p className="font-bold">GH₵{fmt(grandTotal)}</p></div>
                    </div>
                ...
                {status === 'shipping_paid' && (
                  <div className="rounded-xl border border-[var(--color-success)] bg-[var(--color-success-light)] p-4 space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-sm font-semibold text-[var(--color-success)]">Customer confirmed payment</p>
                    </div>
                    ...
                    <p className="text-xs text-[var(--color-text-muted)]">Verify GH₵{fmt(shippingFee)} payment then mark delivered.</p>
                    <button
                      disabled={loading[order.id + '_paid']}
                      onClick={(e) => { e.stopPropagation(); handleMarkPaid(order) }}
                      className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[var(--color-success)] text-white text-sm font-semibold transition-all disabled:opacity-50"
                    >
                      {loading[order.id + '_paid'] ? <Loader2 className="h-4 w-4 animate-spin" /> : <PackageCheck className="h-4 w-4" />}
                ...
                {status === 'delivered' && (
                  <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-sm font-semibold text-[var(--color-success)]">Order completed</p>
                    </div>
                    <div className="grid grid-cols-3 gap-3 text-sm">
                      ...
                      <div><p className="text-xs text-[var(--color-text-muted)]">Total</p><p className="font-bold text-[var(--color-success)] tabular-nums">GH₵{fmt(grandTotal)}</p></div>
                    </div>
                  </div>
                )}
              </div>
